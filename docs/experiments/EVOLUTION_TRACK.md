> **状态：归档的演进路线图（2026-09-09 撰写）。** 本文成文于自研 KV 栈时代，
> 其中 Batch / Paged KV / Admission 等机制描述以当时的自研实现为前提；
> 2026-09-12 起 Physical KV 与资源边界已交由官方 ChatSession 承担（见 README_base §5.1.1、§六）。
> 阅读方式：S1 Controlled Batch 在当前依赖组合下维持 NO-GO；
> S2/S3 与「第二十章 借鉴边界」「第二十一章 不照搬」是后续演进的参考思想库，
> 不是当前实现说明书。机制落地前必须按第四章演进纪律重新走完整管道。
